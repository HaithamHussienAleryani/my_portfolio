...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`mx-auto rounded-full size-[110px] lg:size-[140px] text-black font-medium text-center cursor-pointer origin-center ${className}`}
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute p-1 lg:p-2 inline-block inset-0 text-sm lg:text-md transition-all duration-500 ease-[cubic-bezier(0,0,0,1)]"
